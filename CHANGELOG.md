...
**An economy rebase, for people who find 200% markups silly.** F1 config `Economy > Profile`: Vanilla, Tight, Harsh or Custom. It squeezes the built-in gap between what you pay wholesale and what customers call a fair price, trims what single cards fetch, makes customers judge a markup more harshly, and nudges restock costs up. Works on an existing save at once (nothing in the save is rewritten - turn it off and vanilla is back), applies in single player too, and guests see the host's prices. Switch live from the cheat menu.

**Difficulty that follows the player count.** A host-side profile (Off / Relaxed / Normal / Busy / Chaos / Custom, F1 config `Difficulty`) scales how many customers the shop draws, how fast they arrive and how much they carry - and grows the crowd for every extra player in the session (35% per player by default), since two pairs of hands clear a queue twice as fast. Switch profiles live from the cheat menu. Explicit `Population` settings still override it.
