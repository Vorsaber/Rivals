...
**The Tetramon card game works in co-op.** Both players can sit down against a waiting customer and play - the host as always, and now the guest too: right-click the table, the host books the seat, the battle runs on the guest's own screen with the shop's decks, and when it ends the customer stands up and leaves for everyone. Won gift packs land in the winner's hands. When the host is playing, the guest can walk up and watch: the board, both guardians, every evolution stack, HP and deck counts are mirrored live, and the other player actually sits at the table instead of standing on the chair. The host does not yet see the guest's board (only the guest seated), and you cannot sit at a tournament table as the guest.
