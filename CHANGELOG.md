...
**The Tetramon card game works in co-op.** Both players can sit down against a waiting customer and play - the host as always, and now the guest too: right-click the table, the host books the seat, the battle runs on the guest's own screen with the shop's decks, and when it ends the customer stands up and leaves for everyone. Won gift packs land in the winner's hands. Whoever is playing, the other player can walk up and watch: the board, both guardians, every evolution stack, HP and deck counts are mirrored live, and the player actually sits at the table instead of standing on the chair. Someone joining mid-battle sees it straight away. Decks are shared shop property: either player can build or change one at the Workbench (one editor at a time - the other gets a short notice while it's in use), and every deck shows up on both PCs. Each player picks their own deck to battle with. The shop's tournament entry is open to the guest too: sign up from the phone, and on tournament day the assigned table seats you like it would the host, with the result going onto the bracket for everyone. One entry per shop, so whoever signs up first has it. The guest can also schedule, cancel and set the prizes for the shop's tournament from their own phone - the host's data updates the moment they confirm.

**Host vs guest card battles (experimental).** The host right-clicks an empty play table to wait; the guest right-clicks the same table and the match starts on both screens, each playing their own selected deck against the other's. Both PCs run the real game engine in lockstep - the other player's moves replace the AI - so it plays exactly like a customer match, mulligans, effects, searches and all. No gift packs from a PvP match. If the two engines ever disagree the log says so; leave the table and sit again.
...
**Up to 8 players.** `Network > MaxPlayers` (default 4) sets the Steam lobby size and refuses LAN joins past it. Everything relays through the host, so beyond four the host's upload is the practical limit.

**Talk, point, and see what the other one's up to.** A chat line (T; Enter sends) and a "need you here" ping (H) that names the nearest register, workbench, play table or the warehouse door and puts "over here!" above your head. The co-op window's session panel now lists each player with what they're doing (in a battle, editing decks, at the register, on the phone, entered in the tournament) and their session counters (packs opened, battles, checkouts), shows the guest what difficulty and economy the host is running, and keeps the last chat lines with an input. Cheat menu additions: duplicate any deck, a deck made at a guest's request is selected for that guest (not the host), and live sliders for customer cap, arrival rate, per-player crowd growth and staff cost that apply without a relaunch. A guest's selected deck is remembered by name across sessions.

**Ending the day is a vote.** After closing time a guest's Enter marks them ready (Enter again to cancel) and the host sees who's ready; the host's first Enter waits for everyone with a notice on both sides, a second Enter ends the day regardless. `Coop > SleepVote` turns it off. And when a join is refused for a version or mod-set mismatch, the message can now say where to get the host's build (`Network > UpdateHint` on the host).

**Two companion plugins, standalone.** `TcgEconomy.dll` and `TcgDifficulty.dll` are separate BepInEx mods that work without co-op; CardShopCoop finds them if installed and does the co-op part.
- *TCG Economy Rebase* (`TcgEconomy.dll`, config section `Economy`): Vanilla, Tight, Harsh or Custom. Squeezes the built-in gap between what you pay wholesale and what customers call a fair price, trims what single cards fetch, makes customers judge a markup more harshly, raises restock costs and the daily rent and electricity bills (wages untouched). Works on an existing save at once - nothing in the save is rewritten, turn it off and vanilla is back. In co-op the host's profile applies to everyone.
- *TCG Difficulty* (`TcgDifficulty.dll`, config section `Difficulty`): Off / Relaxed / Normal / Busy / Chaos / Custom over how many customers the shop draws, how fast they arrive and how much they carry. With CardShopCoop the crowd also grows for every extra player (35% per player by default), since two pairs of hands clear a queue twice as fast; explicit `Population` settings still override it. Hiring staff and their daily wages also scale with the player count (`StaffCostPerPlayer`, default: two players pay double, three triple - extra hands make hired staff a luxury).
- Both switch live from the cheat menu when installed.

**A test-rig cheat menu (F4, host only).** Money, shop level, every license, the tutorial, a full set of any expansion, a ready-made 50-card deck, any delivery box, any boxed furniture - each through the game's own path, so the guest sees the result like a real purchase. Turn it off with Cheats > Enabled in the F1 config. The guest has the same menu; their buttons are requests the host runs (furniture lands in front of the guest), and the host can refuse them with Cheats > AllowGuestRequests.
...
When the host joins their own tournament, the guest's pairing board shows them where vanilla would. The guest can no longer accidentally kick a table someone is playing at.
