    ...
    public partial class CoopCore
    {
        private void RegisterDomainRoutes()
        {
            ...
            _messageRouter.Register<TradeStateMessage>((context, message) =>
            {
                ...
                MessagePolicy.ClientOnlyInGame, false, heal: () => _world.RequestResyncCoalesced());
            _messageRouter.Register<BattleSitMessage>((context, message) =>
            {
                if (Role != CoopRole.Host || !InGameLevel())
                    return;
                if (message is BattleSitMessage sit)
                    _guestBattle.HostApplySit(sit, context.ConnectionId);
                return;
            },
                MessagePolicy.HostOnlyInGame, false);
            _messageRouter.Register<BattleExitMessage>((context, message) =>
            {
                if (Role != CoopRole.Host || !InGameLevel())
                    return;
                if (message is BattleExitMessage exit)
                    _guestBattle.HostApplyExit(exit, context.ConnectionId);
                return;
            },
                MessagePolicy.HostOnlyInGame, false);
            _messageRouter.Register<BattleSitResultMessage>((context, message) =>
            {
                if (Role != CoopRole.Client || !InGameLevel())
                    return;
                if (message is BattleSitResultMessage result)
                    _guestBattle.ClientApplySitResult(result);
                return;
            },
                MessagePolicy.ClientOnlyInGame, false);
            _messageRouter.Register<DeckStateMessage>((context, message) =>
            {
                if (Role != CoopRole.Client || !InGameLevel())
                    return;
                if (message is DeckStateMessage deckState)
                    _decks.ClientApplyState(deckState);
                return;
            },
                MessagePolicy.ClientOnlyInGame, false);
            ...
            _messageRouter.Register<TournamentEntryMessage>((context, message) =>
            {
                if (Role != CoopRole.Host || !InGameLevel())
                    return;
                if (message is TournamentEntryMessage entry)
                    _tournament.HostApplyEntry(entry, context.ConnectionId);
                return;
            },
                MessagePolicy.HostOnlyInGame, false);
            ...
            _messageRouter.Register<SocialMessage>((context, message) =>
            {
                if (message is SocialMessage social)
                    _social.Apply(social, context.ConnectionId);
                return;
            },
                MessagePolicy.Any, false, heal: null);
            _messageRouter.Register<ActivityStatusMessage>((context, message) =>
            {
                if (message is ActivityStatusMessage status)
                    _social.ApplyStatus(status, context.ConnectionId);
                return;
            },
                MessagePolicy.Any, false, heal: null);
            ...
            _messageRouter.Register<CheatRequestMessage>((context, message) =>
            {
                if (Role != CoopRole.Host || !InGameLevel())
                    return;
                if (message is CheatRequestMessage req)
                    UI.CheatMenu.HostApplyRequest(req, context.ConnectionId);
                return;
            },
                MessagePolicy.HostOnlyInGame, false);
            _messageRouter.Register<CheatResultMessage>((context, message) =>
            {
                if (Role != CoopRole.Client)
                    return;
                if (message is CheatResultMessage res)
                    UI.CheatMenu.ClientApplyResult(res);
                return;
            },
            ...
            _messageRouter.Register<TournamentEntryResultMessage>((context, message) =>
            {
                if (Role != CoopRole.Client || !InGameLevel())
                    return;
                if (message is TournamentEntryResultMessage result)
                    _tournament.ClientApplyEntryResult(result);
                return;
            },
                MessagePolicy.ClientOnlyInGame, false);
            ...
            _messageRouter.Register<BattleStateUpMessage>((context, message) =>
            {
                if (Role != CoopRole.Host || !InGameLevel())
                    return;
                if (message is BattleStateUpMessage up)
                    _battle.HostApplyUp(up, context.ConnectionId);
                return;
            },
                MessagePolicy.HostOnlyInGame, false);
